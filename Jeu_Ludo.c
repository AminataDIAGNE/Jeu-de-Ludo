...
void enfantParleAuPere(int* tab_pipes_pere_fils[N],int *tab_pipes[N],int* p1){
	...
	for(int i=1 ;i<N+1; i++){
		...
		close(tab_pipes_pere_fils[i][1]);
		int nb_lu = read(tab_pipes_pere_fils[i][0], &A, sizeof(A));
		printf("Le fils numero %d  a reçu la valeur: %d du pere et \n",i,A);
		close(tab_pipes_pere_fils[i][0]);
...
/******************les commandes ci dessous dans le if sont executées que par le fils qui joue********************/
			...
			printf("Le fils numero %d a envoyé la valeur %d au processus pere \n",i,i);
			close(tab_pipes[i][1]);
